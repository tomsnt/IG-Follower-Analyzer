...
class FileParser {
    ...
    static func parseCSVContent(_ csvContent: String) -> [InstagramUser] {
        ...
        for (index, line) in lines.enumerated() {
            ...
            if components.count >= 3 {
                ...
                // Parse timestamp if available (from Chrome extension CSV)
                var timestamp: Date? = nil
                if components.count >= 5 {
                    let timestampString = components[4].trimmingCharacters(in: CharacterSet(charactersIn: "\""))
                    timestamp = parseISO8601Date(timestampString)
                    if let parsedDate = timestamp {
                        print("📅 Parsed date for \(username): \(parsedDate)")
                    } else {
                        print("⚠️ Failed to parse date for \(username): \(timestampString)")
                    }
                } else {
                    print("⚠️ No timestamp column found for \(username)")
                }
    ...
    private static func parseISO8601Date(_ dateString: String) -> Date? {
        ...
        // Fallback per formati alternativi
